...
namespace InternalClasses
{
	public static class ExtensionClass
	{
		...
		public static void DeleteOldFilesInDir(this DirectoryInfo dir_path, long desired_folder_size)
		{
			List<FileInfo> list = dir_path.GetFiles().ToList<FileInfo>();
			long length = (long)0;
			list.ForEach((FileInfo a) => length = length + a.Length);
			if (length > desired_folder_size)
			{
				...
				try
				{
					...
					while (length >= desired_folder_size);
				}
				finally
				{
		...
		public static void DeleteOldFilesInDir(this DirectoryInfo dir_path, long desired_folder_size, bool recursive)
		{
			if (!recursive)
			{
				dir_path.DeleteOldFilesInDir(desired_folder_size);
			}
			List<FileInfo> list = dir_path.GetFiles("*.*", SearchOption.AllDirectories).ToList<FileInfo>();
			double length = 0;
			list.ForEach((FileInfo a) => length = length + (double)a.Length);
			if (length > (double)desired_folder_size)
			{
				list.Sort((FileInfo a, FileInfo b) => b.CreationTime.CompareTo(a.CreationTime));
				double num = 0;
				List<FileInfo> fileInfos = list.TakeWhile<FileInfo>((FileInfo a) => {
					num = num + (double)a.Length;
					return num < (double)desired_folder_size;
				}).ToList<FileInfo>();
        ...
        public static void DeleteOldFilesInDir(this DirectoryInfo dir_path, long desired_folder_size, int del_files_count)
        {
            List<FileInfo> list = dir_path.GetFiles("*.*", SearchOption.AllDirectories).ToList<FileInfo>();
            double length = 0;
            list.ForEach((FileInfo a) => length = length + (double)a.Length);
            ...
            {
                list.Sort((FileInfo a, FileInfo b) => a.CreationTime.CompareTo(b.CreationTime));
                var fileInfos1 = list.GetRange(0, del_files_count);

                fileInfos1.ForEach((FileInfo a) =>
                {
                        a.Delete();
                });
            }
