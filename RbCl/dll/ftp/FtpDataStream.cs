...
namespace Ftp
{
	...
	public class FtpDataStream : Stream, IDisposable
	{
		...
		public override void Close()
		{
			if(!IsClosed) {
                try
                { 
                   // MDIParentMain.Log("-----------11");
                    CloseConnection();
                    //MDIParentMain.Log("-----------12");
                    //MDIParentMain.Log("----------- " + m_ctrl.LastResponse.Message);
                    m_ctrl.RefreshResponse();
                 //   MDIParentMain.Log("-----------13");
                    m_ctrl.Session.EndDataTransfer();
                 //   MDIParentMain.Log("-----------14");
                }
	...
	{
		internal FtpInputDataStream(ControlChannel ctrl, TcpClient client) : base(ctrl, client)
		{
		}
		public override void Write(byte[] buffer, int offset, int count)
		{
	...
	{
		internal FtpOutputDataStream(ControlChannel ctrl, TcpClient client) : base(ctrl, client)
		{
		}
		public override int Read(byte[] buffer, int offset, int count)
		{
