...
namespace RBClient.Classes
{
    public class StaticHelperClass
    {
        ...
        public static void SetControlAvailability(object parentwindow, string cname, bool EnabledStatus)
        {
            object o = StaticHelperClass.ReturnClassItemValue(parentwindow, cname,
                    BindingFlags.Instance | BindingFlags.NonPublic);

            if (o != null)
            {
                StaticHelperClass.SetClassItemValue(o, "Enabled", EnabledStatus);
            }
        }

        public T ConvertType<T>(object prop, T default_value) //where T:struct
        {
            //заполнить поля
            T val = default_value;
            object o = prop;
        ...
        /// <returns></returns>
        public static Color ReturnColor(string color_str)
        {
        ...
        /// <param name="prop_name"></param>
        /// <returns></returns>
        public static object ReturnClassItemValue<T>(T nclass, string prop_name,BindingFlags battr) where T : new()
        {
            try
            {
                Type type = nclass.GetType();
                FieldInfo pi = type.GetField(prop_name,battr);
        ...
        /// <param name="prop_name"></param>
        /// <param name="prop_value"></param>
        public static void SetClassItemPropValue<T>(T nclass, string prop_name, object prop_value) where T : class
        {
            try
            {
                Type type = nclass.GetType();
        ...
        public static object CallMethodEx<T>(T nclass, string method_name, object[] param_s,out Exception out_exception) where T : new()
        {
            try
            {
                object o = null;
        ...
        public static object CallMethod(object nclass, string method_name, object[] param_s)
        {
            try
            {
                object o = null;
        ...
        public static object ExecuteMethodByName(object o, string method_name, object[] param_s)
        {
            try
            {
                ...
                MethodInfo method=null;
                int pcount = 0;
                if (param_s != null) pcount = param_s.Count();

                foreach (MethodInfo mi in mi_list)
                {
                    if (mi.Name == method_name && mi.GetParameters().Count() == pcount)
                    {
                        method = mi; break;
                    }
                }                
        ...
        public static object ExecuteTMethodByName(Type o, string method_name,BindingFlags bindingFlags, object[] param_s)
        {
            try
            {
                Type type = o;
                List<MethodInfo> mi_list = type.GetMethods(bindingFlags).ToList();
                MethodInfo method = null;
        ...
        /// <param name="avoided_props">исключенные из выборки</param>
        /// <returns></returns>
        internal static List<FieldInfo> ReturnModelFieldsList(object model,string[] avoided_props)
        {
            try
            {
                Type type = model.GetType();
                var list = type.GetFields().ToList();
                if (avoided_props.NotNullOrEmpty())
                {
                    foreach (var o in avoided_props)
                    {
                        var ooo = list.WhereFirst(oo => oo.Name.Equals(o, StringComparison.OrdinalIgnoreCase));
                        if (ooo != null)
                        {
                            list.Remove(ooo);
                        }
