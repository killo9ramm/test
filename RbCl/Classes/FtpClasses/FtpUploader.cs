...
namespace ftpUpLoader
{
    class FtpUploader : LoggerBase
    {
        ...
        public List<string> makeRemoteFolders(string rootFolder,string currentFolder)
        {
            string folder = currentFolder.Substring(currentFolder.IndexOf(rootFolder));
            return folder.Split(new char[] { Path.AltDirectorySeparatorChar }, StringSplitOptions.RemoveEmptyEntries).ToList<string>();
        }

        public bool CheckFileExist(FileInfo file,string RootFolder,string CurrentFolder,bool useConnection)
        {
            try
            {
                if (useConnection) Connect(username, password);

                List<string> remote_folders = makeRemoteFolders(RootFolder, CurrentFolder);

                if (remote_folders == null || remote_folders.Count == 0)
                    throw new ArgumentOutOfRangeException("remote_folders", "Внутренняя структура папок не должна быть пуста");
                ...
                //получить текущую директорию
                FtpFile ftpFile = m_client.CurrentDirectory.FindFile(file.Name);
                if (ftpFile.IsFile)
                {
                    if (ftpFile.Size == file.Length)
                        return true;
                }
                return false;
            ...
            {
                Log(ex, "Не получилось отправить файл " + file.FullName + " объект " + Serializer.JsonSerialize(m_client));
                return false;
            }
            finally
            {
