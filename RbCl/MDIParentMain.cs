...
namespace RBClient
{
    ...
    public partial class MDIParentMain : Form
    {
        ...
        public MDIParentMain()
        {
            ...
            //WindowStarter.Открыть_смена_и_обеды_дата(DateTime.Now);
            //return;
...
//            StaticConstants.WebServiceSystem.RecieveLogFilesList(
         ...
         //   return;
            ...
            //WindowsController.Обновить_и_отправить_табель_на_основе_смены(doc);
        ...
        //    return;
          ...
          //  object o54 = find.Invoke(o1, new object[] { key5 });
            ...
            catch (Exception ex)
            {
                ...
                WpfCustomMessageBox.Show("Произошла серьезная ошибка в MdiParentMainError!!! Свяжитесь с технической поддержкой!!!");
            }
        }
            ...
            where T : ModelClass, new()
        {
                ...
                Log(String.Format("Закончили удалять файлы type {0} count {1} ",typeof(T).Name.ToString(),docs.Count));
        }
...
#if(!DEB)
                #region disabled                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             //установить внутренний параметр удаления документов
                //if (StaticConstants.UpdateHelper.IsVersion("5.6.5.5") && CParam.AppCity == 1)
                //{
        ...
        private void toolStripButton_Transfer_Click(object sender, EventArgs e)
        {
            ...
            WindowStarter.ОткрытьВозврат(DateTime.Now);
                    ...
                    if (!_file.Name.EndsWith(".tmp"))
                    {
                        ...
                        if (_file.Name.Substring(_file.Name.Length - 3, 3) == "xml")
                        {
                            ...
                            switch (_s[1])
                            {
                                ...
                                case "39": // справочник причины списания гп
                                    Log("Получен справочник шаблонов группировки документов");
                                    log.Info("Получен справочник шаблонов группировки документов");
        ...
        private void ExchangeFTP(FtpSession m_client)
        {
            ...
            try
            {
                ...
                foreach (DataRow _row in _table_folders.Rows)  //проходим по каждому теремку
                {
                             ...
                             if (_table_doc != null && _table_doc.Rows.Count > 0)
                             {
                                 ...
                                 foreach (DataRow _row_doc in _table_doc.Rows)
                                 {
                                     ...
                                     try
                                     {
                                         ...
                                         Log("Отправили файл код" + docId);
                                         log.Info("Отправили файл код" + docId.ToString());
                                         m_need_refresh_docjournal = true;
                                         StaticConstants.MainGridUpdate();
                                     }
                                     catch (Exception ex)
                                     {
                                         Log(ex, "SendFile exception. Не удалось отправить файл ID="+docId+" Type="+doc_type);
                                     }
                                 }
                             }
...
//#endif
        ...
        public bool IsWorkerEnabled(BackgroundWorker bgw)
        {
            ...
            if(bgw.Equals(backgroundWorker_Inbox))
            {
                ...
                {
                    log.Warn("backgroundWorker_Inbox занят и не может обработать задачу");
                    return false;
                }
            }
            ...
            if (bgw.Equals(backgroundWorker_CheckKKM))
            {
                ...
                {
                    log.Warn("backgroundWorker_CheckKKM занят и не может обработать задачу");
                    return false;
                }
            }
            ...
            if (bgw.Equals(backgroundWorker_Info))
            {
                ...
                {
                    return false;
                    log.Warn("backgroundWorker_Info занят и не может обработать задачу");
                }
            }
            ...
            if (bgw.Equals(backgroundWorker_ZReport))
            {
                ...
                {
                    log.Warn("backgroundWorker_ZReport занят и не может обработать задачу");
                    return false;
                }
            }
            ...
            while (thread.IsAlive)
            {
                ...
                if (backgroundWorker_Inbox.CancellationPending)
                {
                    if (thread.ThreadState != System.Threading.ThreadState.Stopped)
                    {
                        try
                        {
                            Log("backgroundWorker_Inbox_DoWork прекращаем поток backgroundWorker_Inbox_Thread");
                            thread.Abort();
                        }
                        catch (Exception ex)
                        {
        ...
        private void Inbox_exchange_method()
        {
            ...
            try
            {
                ...
                if (!m_error)
                {
                    ...
                    log.Debug("Inbox_DoWork Обмен завершен без ошибок");
                }
                if (m_error)
                {
                    ReportProgressBgw(backgroundWorker_Inbox, 100);
                    Label1.Text = "Сбой обмена";
                    log.Debug("Inbox_DoWork Сбой обмена ftp");
                    //backgroundWorker_Inbox.CancelAsync();   
                }
                ...
                //                #if(!DEB && WEBSRV)   
                log.Debug("Inbox_DoWork начинаем обмен с вебсервисом");

//#if(!DEB)
                askWebService();  // опрашивает webservice и делает записи в базу
//#endif

                log.Debug("Inbox_DoWork обмен с вебсервисом завершен");
                // #endif    
            ...
            {
                Label1.Text = "Обмен завершен";
                log.Debug("Inbox_DoWork Обмен завершен обмен прерван ThreadAbortException");
            }
            catch (Exception exp)
            {
        ...
        private void  backgroundWorker_ZReport_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            try
            {
               ...
               //найти есть ли z-файлы для для выгрузки
                ...
                Log("ZReport_DoWork проверить папку темпа z-отчетов");
...
#if(!DEB)
                Log("ZReport_DoWork проверить папки касс");
                List<string> outPathes = RbClientGlobalStaticMethods.ReturnKKmOutPathes(false);
                if (outPathes == null && outPathes.Count==0)
                {
                    MDIParentMain.Log("ZReport_DoWork outPathes пусть. Нет касс в сети! Stop!");
                    return;
                }
                ...
                Log("ZReport_DoWork начинаем обработку z-отчетов");
                int doc_id=0;
                if (x_files_missing.Count > 0)
                {
                    ...
                    x_files_missing.ForEach(a=>{
                        Log("ZReport_DoWork обрабатываем файл "+a.Name);
                        new CustomAction(o_o =>
                            {
                                try
                                {
                                    ...
                                    Log("ZReport_DoWork подготовляем пакет " + a.Name);
                                    //получить нужные файлы для z-отчета
                                    List<FileInfo> files_to_z = zmaker.prepareZpackage(a);
                                    Log("ZReport_DoWork создаем архив " + a.Name);
                                    //создать z-отчеты
                                    FileInfo zrep = zmaker.makeZreport(files_to_z, teremok.teremok_1C + "_X" + num_name + ".zip", z_temp_dir);
                                    Log("ZReport_DoWork создали z-отчет " + zrep.Name);

                                    //копируем в outbox
                                    if (zrep != null)
                                    {
                                        zrep.CopyTo(Path.Combine(dir_outbox.FullName, zrep.Name), true);
                                        Log("ZReport_DoWork скопировали в outbox " + zrep.Name);
                                    }
                                    //пропарсить x_otchet
                                    Log("ZReport_DoWork парсим отчет " + a.Name);
                                    doc_id=zreporthelper.ZReportParse(a);
                                }
                                catch (Exception ex)
                                {
                                    Log(ex, "ZReport_DoWork Ошибка создания z_отчета " + a.FullName);
                                }
        ...
        private static void Operate_Out_Z_Folder(DirectoryInfo z_back_dir, Func<DirectoryInfo, IEnumerable<FileInfo>> get_missing_x_files, ZreportMaker zmaker, string OutPath)
        {
            ...
            if (x_f_miss_kkm.Count() > 0)
            {
                ...
                //логировать содержимое папки перед сбором отчета
                Log("ZReport_DoWork ищем номер кассы");
                int kkm_num = UpdateClass.GetKkmName(x_f_miss_kkm.First());
                Log("ZReport_DoWork номер кассы " + kkm_num);
                ...
                if (kkm_num != -1)
                {
                    Log("ZReport_DoWork логировать содержимое папки до сбора отчетов");
                    UpdateClass.kkm_LogOutFilesIn(StaticConstants.Main_Teremok_1cName, kkm_num, "5", OutPath);
                    Log("ZReport_DoWork логировать содержимое папки до сбора отчетов завершено");
                }


                foreach (var b in x_f_miss_kkm)
                {
                    //нотификация об обрабатываемых файлах
                    Log("ZReport_DoWork нотификация об обрабатываемых файлах " + b.Name);
                ...
                {
                    //получить нужные файлы для z-отчета
                    Log("ZReport_DoWork получить нужные файлы для z-отчета " + b.Name);
                    List<FileInfo> files_to_z = zmaker.prepareZpackage(b);

                    Log("ZReport_DoWork перемещаем файлы z-отчета в папку " + z_back_dir.FullName);
                    files_to_z.ForEach(c =>
                    {
                        c.MoveWithReplase(z_back_dir.FullName);
                        Log("ZReport_DoWork переместил " + c.Name);
                    });
                });

                //логировать содержимое папки после сбора отчетов
#if(!DEB)
                if (kkm_num != -1)
                {
                    Log("ZReport_DoWork логировать содержимое папки после сбора отчетов");
                    UpdateClass.kkm_LogOutFilesOut(StaticConstants.Main_Teremok_1cName, kkm_num, "5", OutPath);
                    Log("ZReport_DoWork логировать содержимое папки после сбора отчетов завершено");
                }
#endif
            }
        }
