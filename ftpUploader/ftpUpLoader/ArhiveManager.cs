...
namespace ZippArhiveHelper
{
    ...
    public class ArhiveManager
    {
        ...
        internal static bool UnArchiveFile(FileInfo file, DirectoryInfo dest_dir)
        {
            try
            {
                FastZip fz = new FastZip();
                fz.ExtractZip(file.FullName, dest_dir.FullName, "");
                return true;
            }catch(Exception ex)
            {
                Log(ex.Message +" Не получилось разархивировать "+file.FullName);
                return false;
            }
        }
    }
